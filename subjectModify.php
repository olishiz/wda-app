...
if (isset($_POST['updateSubject'])) {
  ...
  if ($idStatus == "true" && $nameStatus == "true") {
    ...
    if ($conn->query($sql) === true) {
      $sqlStatus = "success";
      $msg = "Update Subject Record Successful!";
    } else {
      $sqlStatus = "failure";
      $msg = "Update Subject Record Fail!";
    }
...
if (isset($_POST['removeSubject'])) {
  ...
  if ($conn->query($sql)) {
    $sqlStatus = "success";
    $msg = "Subject Record has been deleted successfully.";
  } else {
    $sqlStatus = "failure";
...
<head>
  ...
  <title>Modify Subject Page</title>
  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper">
      <div class="container-fluid">
        ...
        <div class="card">
            <h5 class="card-header">Modify Subject Records</h5>
            <div class="card-body">
            ...
            ?>
                <p class="card-text">Modify subject records for students.</p>
                <div class="panel-body">
                        <form action="" method="post">
                            <div class="form-group text-left">
                                <label for="">Subject ID</label>  
                                <input class="form-control text-left" placeholder="Subject ID" name="subId" type="text" style="width: 300px;" 
                                value="<?php $sql = "SELECT subjectId FROM subjects WHERE id = $id";
                                ...
                                Note: Start each subject ID with capital letter 'S' and followed by three digits.
                                </small>                                                      
                            </div>
                            <div class="form-group">
                                <label for="">Subject Name</label>                                
                                <input class="form-control" placeholder="Subject Name" name="subName" type="text" style="width: 300px;" 
                                value="<?php 
                                      $sql = "SELECT subjectName FROM subjects WHERE id = $id";
                                      $result = $conn->query($sql);
                                      while ($row = $result->fetch_assoc()) {
                            ...
                            <?php
                            $sql = "SELECT subjectType FROM subjects WHERE id = $id";
                            $result = $conn->query($sql);
                            $input = $s1 = $s2 = "";
                            while ($row = $result->fetch_assoc()) {
                              $input = $row['subjectType'];
                            }
                            if ($input === "Core")
                              $s1 = "checked";
                            else if ($input === "Selective")
                              $s2 = "checked";

                            ?>
                            <label for="">Subject Type</label>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="subType" id="exampleRadios1" value="Core" <?php echo $s1; ?>>
                                <label class="form-check-label" for="exampleRadios1">
                                    Core
                                </label>
                            </div>
                                <div class="form-check">
                                <input class="form-check-input" type="radio" name="subType" id="exampleRadios2" value="Selective" <?php echo $s2; ?>>
                                <label class="form-check-label" for="exampleRadios2">
                                    Selective
                                </label>
                            </div>
                            <br>
                            <input class="btn btn-lg btn-primary btn-block" style="width: 300px;" type="submit" value="Update Subject" name="updateSubject">         
                            <input class="btn btn-lg btn-danger btn-block" style="width: 300px;" type="submit" value="Remove Subject" name="removeSubject">                                                                                                   
                        </form>
